...
class Client(Node):
    ...
    def train(self, round_id: int, num_epochs: int, use_profiler = True, limit_num_training_updates: int = 0):
        ...
        profiling_size = self.config.profiling_time
        ...
        for epoch in range(num_epochs):
            ...
            for i, (inputs, labels) in enumerate(self.dataset.get_train_loader(), 0):
                ...
                if i % self.config.log_interval == 0:
                    self.logger.debug(
                        '[%s] [(%d, %d) %5d] loss: %.3f' % (self.id, round_id, epoch+1, i, running_loss / self.config.log_interval))
                ...
                if self.offloading_decision:
                    # @TODO: Incoorporate offloading_response_id in message
    ...
    def exec_round(self, round_id: int, num_epochs: int, response_id: str, server_ref) -> Tuple[Any, Any, Any, Any, float, float, float]:
        ...
        #TODO: This condition only accounts for the first offloaded model.
        #      Need to account for all the models. Maybe the federator should inform 
        ...
        if self.has_offloading_request:
            ...
            while self.round_alternate_trained_models < self.round_alternate_models_to_train:
                ...
                loss, weights, num_samples = self.train(round_id, num_epochs, False, self.offloading_rem_local_updates[other_client_id])
            ...
            self.offloading_rem_local_updates = {}
            self.offloading_response = {}
