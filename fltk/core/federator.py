...
from fltk.strategy.algorithms.deadlineCall import deadline_callable
from fltk.strategy.algorithms.offloadingCall import offloading_callable
...
class Federator(Node):
    ...
    def __init__(self, id: int, rank: int, world_size: int, config: Config):
        ...
        self.unique_pairs = config.unique_pairs
        self.config = config
    ...
    def exec_final_inference(self):
        self.logger.info("Testing client local test datasets")
        last_model = self.get_nn_parameters()
        
        client_accuracies = []
        for client in self.clients:
            self.message(client.ref, Client.update_nn_parameters, last_model)
            accuracy, _ = self.message(client.ref, Client.test)
            client_accuracies.append(accuracy)    
        
        n_clients = len(self.clients)
        n_clients_to_consider = round(self.config.fairness_percentage * n_clients)
        client_accuracies = np.sort(client_accuracies)
    ...
    def exec_round(self, round_id: int):
        ...
        def training_cb(fut: torch.Future, client_ref: LocalClient, client_weights, client_sizes, num_epochs):
            ...
            self.logger.info(f'Training callback for client {client_ref.name} with accuracy={accuracy}')
            self.clients_test_accuracy.append(accuracy)
        ...
        while len(client_weights) != len(self.response_store):
            time.sleep(0.1)
        ...
        self.logger.info(f'Aggregation duration: {time.time() - aggregation_time} seconds')
