...
class Profiler:
    ...
    forward_embed_end_time: float
    forward_lstm_end_time: float
    backwards_c_end_time: float
    backwards_lstm_end_time: float
    ...
    def __init__(self, rounds: int, feature_layers_ends: int, active: bool = False):
        ...
        self.eb = np.zeros(self.round)
        self.ef = np.zeros(self.round)
    ...
    def attach(self, module: Module):
        ...
        for idx, k in enumerate(kids):
            ...
            if idx == self.feature_layers_ends:
                ...
                handle = k.register_forward_pre_hook(self.pre_forward_embed)
                self.hook_handles.append(handle)
            ...
            if idx == self.feature_layers_ends + 1:
                ...
                self.hook_handles.append(handle)
            if idx == self.feature_layers_ends -1:
                handle = k.register_backward_hook(self.full_backwards_embed)
                self.hook_handles.append(handle)
    ...
    def full_backwards_embed(self, module, grad_input, grad_output):
        self.backwards_lstm_end_time = time.time()
        self.fb[self.batch_idx] =  self.backwards_lstm_end_time - self.backwards_c_end_time
        return None

    def pre_forward_embed(self, other, input):
        self.forward_embed_end_time = time.time()
        self.ef[self.batch_idx] = self.forward_embed_end_time - self.forward_start_time
    
    def full_backwards(self, module, grad_input, grad_output):
        self.backwards_c_end_time = time.time()
        self.cb[self.batch_idx] =  self.backwards_c_end_time - self.backwards_start_time
        return None

    def pre_forward(self, other, input):
        self.forward_lstm_end_time = time.time()
        self.ff[self.batch_idx] = self.forward_lstm_end_time - self.forward_embed_end_time
        # if self.warmup:
        #     return None
    ...
    def signal_forward_end(self):
        ...
        self.cf[self.batch_idx] = self.forward_end_time - self.forward_lstm_end_time
    ...
    def signal_backwards_end(self):
        ...
        self.eb[self.batch_idx] = self.backwards_end_time - self.backwards_lstm_end_time
    ...
    def get_values(self):
        ...
        """
        return self.ef, self.ff, self.cf, self.cb, self.fb, self.eb
    ...
    def aggregate_values(self, from_layer: int = 0):
        ...
        The order is the execution order of forward and then backward propagation of a network
        """
        return self.ef[from_layer:].mean(), self.ff[from_layer:].mean(), self.cf[from_layer:].mean(), self.cb[from_layer:].mean(), self.fb[from_layer:].mean(), \
                self.eb[from_layer:].mean()
