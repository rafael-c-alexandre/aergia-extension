...
def sort_clients(clients: List[dict], reduced: bool = False, slow_first=False):
    if reduced:
        return sorted(clients, key=lambda x: x['rect'], reverse=True)
    return sorted(clients, key=lambda x: x['ect'], reverse=slow_first)
...
def calc_decision_completion_time_multi_offloading(slow_id, fast_id, performance_data: dict, offloading_point: int) -> Tuple[int, int]:
    client_slow = performance_data[slow_id]
    client_fast = performance_data[fast_id]
    def c_s(delta):
        return (client_slow['rl'] - delta) * client_slow['cut'] + (delta * client_slow['rcut'])
    ...
    def c_f(delta):
        # Before was (client_fast['rl'] * client_fast['cut']) + (delta * client_fast['cut'])
        return client_fast['ect']
    return c_s(offloading_point), c_f(offloading_point)

def find_offloading_pair(slow_id, performance_data: dict, similarity_matrix: np.ndarray, offloading_sim_factor: float):
    can_offload = False
    pair_id = None
    pair_c_f = None
    pair_decision = ""
    ...
    for k,v in performance_data.items():
        if k != slow_id:
            ...
            pair_cost = performance_data[potential_fast_id]['ect'] * \
                        (1 + math.log(1 + similarity_matrix[clients.index(slow_id)][clients.index(potential_fast_id) * offloading_sim_factor]))
            
            if performance_data[potential_fast_id]['acb'] > c_f and \
               performance_data[potential_fast_id]['cb'] + c_f < current_lowest_cb and \
                pair_cost < current_lowest_cost:
                
                can_offload = True
                pair_id = potential_fast_id
                pair_c_f = c_f
                current_lowest_cb = performance_data[potential_fast_id]['cb'] + c_f
                current_lowest_cost = pair_cost
        
    if can_offload:
        # For now let's put it to 0
        performance_data[slow_id]['acb'] = 0
        performance_data[pair_id]['acb'] -= pair_c_f
        performance_data[pair_id]['cb'] += pair_c_f   
         
        decision_id = f"{slow_id}-{pair_id}"
        pair_decision = {
                'id': decision_id,
                'from': slow_id,
                'to': pair_id,
    ...
    return performance_data, pair_decision


def cl_algorithm_multi_offloading(performance_data: dict, similarity_matrix: np.ndarray, offloading_sim_factor: float = 1):
    # Leave the offloading sim factor aside for now
    #print(f'Starting offloading cl_algorithm with offloading_sim_factor={offloading_sim_factor}')
    # Mean compute time:
    mct = _mean_compute_time(performance_data)
    for key, item in performance_data.items():
        performance_data[key] = calc_compute_times(item, key)
    client_profiles = [v for k, v in performance_data.items()]
    
    # Initialize available compute bucket in each client
    for k, _ in performance_data.items():
        performance_data[k]['acb'] = 2*mct - performance_data[k]['ect']
        performance_data[k]['cb'] = performance_data[k]['ect']
    
    # Sort client profiles by their speed
    sorted_client_profiles = sort_clients(client_profiles, reduced=False, slow_first=True)
    offloading_decisions = []
    for client in sorted_client_profiles:
        performance_data, decision = find_offloading_pair(client['id'], performance_data, similarity_matrix, offloading_sim_factor)
        if len(decision) > 0:
            offloading_decisions.append(decision)
        
    client_ids = [x['id'] for x in client_profiles]
    print(f'Clients are: {client_ids}')
...
class Offloading(FederatedAlgorithm):
    ...
    def hook_training(self, federator_state, alg_state: dict, training_start_time, round_id) -> bool:
        ...
        client_ids = set()
        for x in federator_state.selected_clients:
            client_ids.add(x.name)
        ...
        if 'inactive' not in alg_state and all(item in federator_state.performance_data for item in client_ids):
            start_offloading_decision_time = time.time()
            ...
            offloading_decisions = \
                cl_algorithm(federator_state.performance_data, sim_matrix, federator_state.config.offloading_similarity_factor) \
                    if federator_state.unique_pairs else cl_algorithm_multi_offloading(federator_state.performance_data, sim_matrix, federator_state.config.offloading_similarity_factor)
            ...
            federator_state.logger.info(f'Offloading decision duration: {time.time() - start_offloading_decision_time} seconds')
